...
The 272 KiB `com.apple.akd` cache was backed up to owner-only private state,
removed, and regenerated by a clean guest reboot. No account database,
keychain, identity, or OpenCore NVRAM was deleted. The sign-in page was reopened
blank for a careful retry with the exact Apple Account spelling and interactive
2FA. A correct local identity cannot bypass an Apple-side account restriction,
and repeated identity rotation or automated credential attempts make diagnosis
worse. If the same Anisette failure survives this clean retry, an OpenCore NVRAM
reset is a separate, explicit operator decision—not an automatic next command.
